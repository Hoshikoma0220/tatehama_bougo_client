...
#pragma warning restore CS0067
        ...
        public RetsubanWindow()
        {
            InitializeComponent();
            this.Load += Loaded;
            this.FormClosed += RetsubanWindow_FormClosed;
            this.Text = "列番設定器 | 館浜電鉄防護無線 - バックアップシステム"; // タイトル設定
            TopMost = true;
            ...
            LCD.Visible = false; // LCD表示を完全無効化
            retsubanLogic.SetCarAction += LCDLogic.SetCar;

            // 編成両数表示を無効化（背景画像に組み込み済み）
            Car_2.Visible = false;
            Car_1.Visible = false;

            // 時刻表示を無効化（背景画像に組み込み済み）
            Time_h2.Visible = false;
            Time_h1.Visible = false;
            Time_m2.Visible = false;
            Time_m1.Visible = false;
            Time_s2.Visible = false;
            Time_s1.Visible = false;

            // オーバーレイ処理は削除しました
        }

        private void RetsubanWindow_FormClosed(object sender, FormClosedEventArgs e)
        {
            // ウィンドウを閉じても音声ループは再開しない
            // Form1で一度だけ管理するため
        }
